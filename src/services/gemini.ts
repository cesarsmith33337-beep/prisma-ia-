...
const candleSchema = {
    ...
    properties: {
        ...
        isFairValueGap: { type: Type.BOOLEAN, description: "True if this candle is part of a 3-candle Fair Value Gap." },
        closeInsideGap: { type: Type.BOOLEAN, description: "True if this candle's close is inside a previously identified FVG." },
        isBounce: { type: Type.BOOLEAN, description: "True if this candle shows a clear rejection/bounce off a key level like VWAP." }
    },
};
...
const chartDataSchema = {
    ...
    properties: {
        ...
        ma3: { type: Type.NUMBER, description: "O valor da Média Móvel de 3 períodos para a última vela." },
        orderBlocks: { 
            type: Type.OBJECT, 
            properties: {
                buy: { type: Type.ARRAY, items: { type: Type.NUMBER } },
                sell: { type: Type.ARRAY, items: { type: Type.NUMBER } }
            },
            description: "Preços de order blocks de compra e venda visíveis."
        },
        deltaHistory: { type: Type.ARRAY, items: { type: Type.NUMBER }, description: "Os 2 últimos valores do Delta Volume (positivo para compra, negativo para venda)." },
        vwap: { type: Type.NUMBER, description: "O valor atual do VWAP se visível." },
        sarHistory: { type: Type.ARRAY, items: { type: Type.NUMBER }, description: "Os 2 últimos valores do Parabolic SAR." },
        currentExpiryAvailable: { type: Type.NUMBER, enum: [1, 2, 5], description: "O tempo de expiração (1, 2, ou 5 min) que está atualmente disponível para entrada na corretora, se visível." }
    },
    required: ["asset", "candles", "swingHighs", "swingLows"]
};
...
- candles: Um array com as últimas 10 velas visíveis, da mais antiga para a mais recente. Para cada vela, extraia todos os campos do schema.
...
- ma3: O valor da média móvel de 3 períodos.
- orderBlocks: Níveis de preço onde existem order blocks de compra (buy) e venda (sell).
- deltaHistory: Os dois últimos valores de delta de volume (se visível).
- vwap: O valor atual do VWAP (se visível).
- sarHistory: Os dois últimos valores do Parabolic SAR (se visível).
- currentExpiryAvailable: O tempo de expiração (1, 2 ou 5) disponível na corretora (se visível).`;
